...
function AdminHandPickedProductsPage() {
  ...
  const saveProduct = async () => {
    ...
    if (editingId) {
      ...
      setSaving(true);
      try {
        await updateAdminHandPickedProduct(editingId, {
          ...previews[0],
    ...
    const results = await Promise.allSettled(
      previews.map((product, index) => createAdminHandPickedProduct({
        ...product,
        category_id: form.category_id ? Number(form.category_id) : null,
        country: form.country,
    ...
    if (succeeded) {
      toast.success(`Added ${succeeded} hand-picked product${succeeded > 1 ? "s" : ""}.${failedPromises.length ? ` ${failedPromises.length} failed.` : ""}`);
      ...
      {modalOpen ? (
        <div className="orders-modal">
          ...
          <div className="orders-modal__card admin-modal">
            <h3>{editingId ? "Edit Hand-Picked Product" : "Add Hand-Picked Products"}</h3>
