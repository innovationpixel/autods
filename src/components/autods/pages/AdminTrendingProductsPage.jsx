...
import {
  createAdminTrendingProduct,
  deleteAdminTrendingProduct,
...
function AdminTrendingProductsPage() {
  ...
  const loadProducts = () => {
    getAdminTrendingProducts()
      ...
      .catch(() => toast.error("Failed to load trending products."))
  ...
  const confirmDeleteProduct = async () => {
    ...
    setDeleting(true);
    try {
      await deleteAdminTrendingProduct(deleteConfirm.id);
      toast.success("Trending product deleted.");
  ...
  return (
    <section className="admin-page admin-curated-page">
      <header className="admin-page__hero admin-page__hero--split">
        <div>
          <span className="admin-page__eyebrow"><LuChartLine /> Trending Products</span>
          <h1>Manage Trending Products</h1>
          <p>Paste an AliExpress product link, target a category and country, and it will show up in users' Trending Products feed for that marketplace.</p>
        </div>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {products.length ? products.map((product) => (
                ...
                <tr>
                  <td colSpan={7} className="admin-table__empty">No trending products yet.</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default AdminTrendingProductsPage;
